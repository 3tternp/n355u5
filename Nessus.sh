...
curl --request GET \
  ...
  --output 'Nessus-10.6.3-debian10_amd64.deb' &>/dev/null
{ if [ ! -f Nessus-10.6.3-debian10_amd64.deb ]; then
  echo " o nessus download failed :/ exiting. where are you man ?? Are you living in stoneage ??"
  exit 0
fi }
echo " o installing Nessus.."
dpkg -i Nessus-10.6.3-debian10_amd64.deb  &>/dev/null
rm -r Nessus-10.6.3-debian10_amd64.deb  &>/dev/null
